...
def test_handlers_are_per_instance():
    """Each protocol instance must have its own handler registry.

    A shared class-level dict would cross-dispatch one device's callbacks to
    another (e.g. a gateway, its sub-devices and standalone units all live at
    once), breaking bind/state handling. Regression test for that.
    """
    a = DeviceProtocol2()
    b = DeviceProtocol2()
    ...
    cb_a = MagicMock()
    cb_b = MagicMock()
    a.add_handler(Response.RESULT, cb_a)
    b.add_handler(Response.RESULT, cb_b)

    # The two registries must be distinct objects and not leak into each other
    assert a._handlers is not b._handlers
    assert cb_b not in a._handlers.get(Response.RESULT, [])
    assert cb_a not in b._handlers.get(Response.RESULT, [])

    # Dispatching on b must only call b's callback
    b.packet_received(
        {"pack": {"t": Response.RESULT.value, "opt": ["Pow"], "val": [1]}},
        ("0.0.0.0", 0),
    )
